<h1 class="small-title">Listar Serviços</h1>
<div class="page-content">
    <button class="btn btn--green align-right">Novo Serviço</button>
    <div class="page-content__list">
        <div class="list__list-item">
            <div class="list-item__name">Nome do serviço</div>
            <div class="list-item__show-details">&#9660;</div>
            <div class="list-item__details">
                <div class="details__price"><strong>Preço: </strong>R$ 300,00</div>
                <div class="details__description"><strong>Descrição:</strong> Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ut laoreet sem vel arcu pharetra blandit.</div>
                <div class="details__professionals">
                    <small>Profissionais</small>
                    <div class="professionals__list">
                        <div class="list__professional">Nome do profissional</div>
                    </div>
                </div>
                <div class="details__actions">
                    <button class="btn btn--green align-right">Editar</button>
                    <button class="btn btn--red align-right">Excluir</button>
                </div>
            </div>
        </div>

        <div class="list__list-item">
            <div class="list-item__name">Nome do serviço</div>
            <div class="list-item__show-details">&#9660;</div>
            <div class="list-item__details">
                <div class="details__price"><strong>Preço: </strong>R$ 300,00</div>
                <div class="details__description"><strong>Descrição:</strong> Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ut laoreet sem vel arcu pharetra blandit.</div>
                <div class="details__professionals">
                    <small>Profissionais</small>
                    <div class="professionals__list">
                        <div class="list__professional">Nome do profissional</div>
                    </div>
                </div>
                <div class="details__actions">
                    <button class="btn btn--green align-right">Editar</button>
                    <button class="btn btn--red align-right">Excluir</button>
                </div>
            </div>
        </div>

        <div class="list__list-item">
            <div class="list-item__name">Nome do serviço</div>
            <div class="list-item__show-details">&#9660;</div>
            <div class="list-item__details">
                <div class="details__price"><strong>Preço: </strong>R$ 300,00</div>
                <div class="details__description"><strong>Descrição:</strong> Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ut laoreet sem vel arcu pharetra blandit.</div>
                <div class="details__professionals">
                    <small>Profissionais</small>
                    <div class="professionals__list">
                        <div class="list__professional">Nome do profissional</div>
                    </div>
                </div>
                <div class="details__actions">
                    <button class="btn btn--green align-right">Editar</button>
                    <button class="btn btn--red align-right">Excluir</button>
                </div>
            </div>
        </div>

    </div>
</div>